...
type Role = 'admin' | 'porteiro';

export default function Page() {
  const ADMIN_EMAILS = ['seuemail@empresa.com']; // troque pelo seu e-mail

  const [mode, setMode] = useState<Mode>('Entrada');
  const [userRole, setUserRole] = useState<Role>('porteiro');
  ...
  function resolveRole(userEmail?: string | null): Role {
    const normalized = (userEmail || '').toLowerCase();
    return ADMIN_EMAILS.includes(normalized) ? 'admin' : 'porteiro';
  }
  ...
  async function loadAccesses() {
    ...
    if (error) {
      console.error('Erro ao carregar access_logs:', error);
      setAccesses([]);
      setAllowed(0);
      setBlocked(0);
      return;
  ...
  async function validate(tagValue?: string) {
    ...
    if (error) {
      console.error('Erro ao consultar vehicles:', error);
      setScanMsg('Erro ao consultar o banco.');
      return;
    }
  ...
  async function toggleVehicleStatus(vehicle: Vehicle) {
    ...
    const { error } = await supabase.from('vehicles').update({ status: nextStatus }).eq('id', vehicle.id);
        ...
        <div className="container">
          <div className="topbar">
            <div className="brand">
              ...
              <div>
                <h1>Controle de Acesso Corporativo</h1>
                <p>{companyName} • Painel da portaria</p>
              </div>
            </div>
            ...
            <div className="content">
              <div className="split">
                <div>
                  ...
                  <div className="stats">
                    <div className="stat"><div className="k">Veículos cadastrados</div><div className="v">{vehicleCount}</div></div>
                    <div className="stat"><div className="k">Liberações</div><div className="v">{allowed}</div></div>
                    <div className="stat"><div className="k">Bloqueios</div><div className="v">{blocked}</div></div>
                  </div>
                </div>
                  ...
                  {accesses.map((item, index) => (
                    <div className="item" key={`${item.id}-${index}`}>
                      <div>
                        ...
                        <span>
                          {item.created_at ? new Date(item.created_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) : '--:--'} • {item.plate} • {item.name}
                        </span>
                      </div>
